...
export function SterlingGateNavigation() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const menuButtonTextsEls = Array.from(menuButton?.querySelectorAll('p') ?? []);
      const menuButtonIconEl = menuButton?.querySelector('.menu-button-icon') as Element | null;
      ...
      if (isMenuOpen) {
        ...
        tl.set(navWrap, { display: 'block' })
          .set(menu, { xPercent: 0 }, '<')
          .fromTo(menuButtonTextsEls, { yPercent: 0 }, { yPercent: -100, stagger: 0.2 })
          .fromTo(menuButtonIconEl ?? [], { rotate: 0 }, { rotate: 315 }, '<')
        ...
        tl.to(overlay, { autoAlpha: 0 })
          ...
          .to(menuButtonTextsEls, { yPercent: 0 }, '<')
          .to(menuButtonIconEl ?? [], { rotate: 0 }, '<')
